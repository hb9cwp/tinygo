...
$(LLVM_BUILDDIR): $(LLVM_BUILDDIR)/build.ninja ## Build LLVM
	cd $(LLVM_BUILDDIR) && ninja -j 4 $(NINJA_BUILD_TARGETS)
...
build/wasm-opt$(EXE):
	mkdir -p build
	cd lib/binaryen && cmake -G Ninja . -DBUILD_STATIC_LIB=ON -DBUILD_TESTS=OFF -DENABLE_WERROR=OFF $(BINARYEN_OPTION) && ninja -j 4 bin/wasm-opt$(EXE)
